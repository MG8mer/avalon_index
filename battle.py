...
async def battle(interaction: Interaction, member: nextcord.Member):
  async with aiosqlite.connect("main.db") as db:
    async with db.cursor() as cursor:
      ...
      await cursor.execute('SELECT battle FROM battles WHERE user_id = ?', (interaction.user.id,))
      battle_value = await cursor.fetchone()
      await cursor.execute('SELECT starter_hp FROM battles WHERE user_id = ?', (interaction.user.id,))
      starter_hp_value = await cursor.fetchone()
      await cursor.execute('SELECT reciever_hp FROM battles WHERE user_id = ?', (member.id,))
      reciever_hp_value = await cursor.fetchone()
    await db.commit()
  while starter_hp_value[0] >= 0 and reciever_hp_value >= 0:
    pass
