...
async def battle_embd(interaction: Interaction, member: nextcord.Member, switch, turn, starter_hp_value, reciever_hp_value, rand_mage):
  if rand_mage == 7:
    ...
    attacks = {
      ...
      "Arcaaaayyyynuhhh Maaayyyniuhhhh": {
        "Weak": -63,
        "Normal": -71,
        "Strong": -83,
      },
  ...
  if rand_mage == 7:
    class ChooseFour(nextcord.ui.View):
      ...
      @nextcord.ui.button(label = 'Zaaaaaaahhhppp', style=nextcord.ButtonStyle.blurple)
      ...
      if normal_c[0] == 0:
        @nextcord.ui.button(label = 'Fireballssss', style=nextcord.ButtonStyle.blurple)
      ...
      if special_c[0] == 0:
        @nextcord.ui.button(label = 'Arcaaaayyyynuhhh Maaayyyniuhhhh', style=nextcord.ButtonStyle.blurple)
        async def special(self, button: nextcord.ui.Button, interaction: Interaction):
          move = 'Arcaaaayyyynuhhh Maaayyyniuhhhh'
          async with aiosqlite.connect("main.db") as db:
            async with db.cursor() as cursor:
              if switch == False:
                await cursor.execute(f"INSERT INTO moves (user_id, opponent_id, move_used, turn_num) VALUES (?, ?, ?, ?)", (id_user, member.id, move, turn,))
      ...
      if avalonbless_c[0] == 0:
        @nextcord.ui.button(label = 'Im OFFENDED!, style=nextcord.ButtonStyle.blurple)
  ...
  zap = attacks[3]["Zap"][evaluation[0]]
  fireball = attacks[3]["Fireball"][evaluation[0]]
  arcane_mania = attacks[3]["Arcane Mania"][evaluation[0]]
  biden_blast = attacks[3]["Biden Blast"][evaluation[0]]
  ...
  if rand_mage == 7:
    ...
    embed.add_field( # Field that shows the weak attack for that class and damage according the value of that user's evaluation.
      ...
      value=attacks[3]["Zap"][evaluation[0]],
      inline=True)
    embed.add_field( # Field that shows the normal attack for that class and damage according the value of that user's evaluation.
      ...
      value=attacks[3]["Fireball"][evaluation[0]] ,
      inline=False)
    embed.add_field( # Field that shows the special attack for that class and damage according the value of that user's evaluation.
      ...
      value=attacks[3]["Arcane Mania"][evaluation[0]],
      inline=False)
    embed.add_field( # Field that shows the weak avalon blessing attack for that class and damage according the value of that user's evaluation.
      ...
      value=attacks[3]["Biden Blast"][evaluation[0]] ,
      inline=False)        
embed.set_thumbnail(url="https://replit.com/@mhammouri26/avalonindex#custom_assets/final_mage_nerfed_eE.png")
  else:
    embed = Embed( # Title and description, indicating user to pick a move.
      title = "Moves",
      description = "",
  ...
  if switch == False: # If it's the starter's turn, send the embed in their dm.
    ...
    async with aiosqlite.connect("main.db") as db:
      async with db.cursor() as cursor:
        ...
        if rand_mage == 7:
          if normal_c[0] == 0 and move_final[0] == " Fireballssss":
          ...
          if special_c[0] == 0 and move_final[0] == " Arcaaaayyyynuhhh Maaayyyniuhhhh":
          ...
          if avalonbless_c[0] == 0 and move_final[0] == "    Im OFFENDED!":
